...
func wir_const_int(ref program: WirModule, type_id: WirTypeID, value: UInt128) -> WirValueID {
    return wir_add_value(ref program, WirValue(type_id=type_id, kind=WirValueKind.Integer, owner=0U, index=-1, integer=value, floating=0.0));
}

func wir_const_float(ref program: WirModule, type_id: WirTypeID, value: Float) -> WirValueID {
    return wir_add_value(ref program, WirValue(type_id=type_id, kind=WirValueKind.FloatValue, owner=0U, index=-1, integer=UInt128(0U), floating=value));
}

func wir_const_bool(ref program: WirModule, value: Bool) -> WirValueID {
    let integer: UInt128 = UInt128(0U);
    if value { integer = UInt128(1U); }
    return wir_add_value(ref program, WirValue(type_id=program.bool_type, kind=WirValueKind.BoolValue, owner=0U, index=-1, integer=integer, floating=0.0));
}

func wir_null(ref program: WirModule, type_id: WirTypeID) -> WirValueID {
    return wir_add_value(ref program, WirValue(type_id=type_id, kind=WirValueKind.Null, owner=0U, index=-1, integer=UInt128(0U), floating=0.0));
}

func wir_add_function(ref program: WirModule, name: String, parameters: Vector(WirTypeID), result: WirTypeID, variadic: Bool, linkage: WirLinkage) -> WirFuncID {
    let function_id: WirFuncID = WirFuncID(UInt32(program.arena.functions.length() + 1));
    let type_id: WirTypeID = wir_function_type(ref program, parameters, result, variadic);
    let values: Vector(WirValueID) = [];
    let i: Int = 0;
    while (i < parameters.length()) {
        values.append(wir_add_value(ref program, WirValue(type_id=parameters[i], kind=WirValueKind.FunctionParameter, owner=UInt32(function_id), index=i, integer=UInt128(0U), floating=0.0)));
        i++;
    }
    let address: WirValueID = wir_add_value(ref program, WirValue(type_id=type_id, kind=WirValueKind.Function, owner=UInt32(function_id), index=-1, integer=UInt128(0U), floating=0.0));
...
func wir_add_block(ref program: WirModule, function_id: WirFuncID, name: String, parameters: Vector(WirTypeID)) -> WirBlockID {
    let block_id: WirBlockID = WirBlockID(UInt32(program.arena.blocks.length() + 1));
    let values: Vector(WirValueID) = [];
    let i: Int = 0;
    while (i < parameters.length()) {
        values.append(wir_add_value(ref program, WirValue(type_id=parameters[i], kind=WirValueKind.BlockParameter, owner=UInt32(block_id), index=i, integer=UInt128(0U), floating=0.0)));
        i++;
    }
...
func wir_append(ref program: WirModule, block_id: WirBlockID, opcode: WirOpcode, type_id: WirTypeID, operands: Vector(WirValueID), edges: Vector(WirEdge), location: WirLocation) -> WirValueID {
    ...
    if (type_id != program.void_type) {
        result = wir_add_value(ref program, WirValue(type_id=type_id, kind=WirValueKind.Instruction, owner=UInt32(instruction_id), index=-1, integer=UInt128(0U), floating=0.0));
    }
...
func wir_add_global(ref program: WirModule, name: String, type_id: WirTypeID, initializer: WirValueID, linkage: WirLinkage, is_const: Bool) -> WirGlobalID {
    ...
    let address: WirValueID = wir_add_value(ref program, WirValue(type_id=pointer_type, kind=WirValueKind.Global, owner=UInt32(global_id), index=-1, integer=UInt128(0U), floating=0.0));
