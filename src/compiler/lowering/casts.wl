...
func emit_implicit_cast(ref c: Compiler, val_res: CompileResult, expected_type: Int, pos: Position) -> CompileResult {
    if (!has_result(val_res) || val_res.reg == "") {
        let dummy_reg: String = "0";
        if (is_nullable_reference_type(ref c, expected_type) || is_pointer_type(ref c, expected_type)) {
            dummy_reg = "null";
    ...
    let expected_named: NamedTypeInfo = get_named_type(ref c, expected_type);
    if (has_named_type(expected_named) && has_func(c.curr_func) &&
        c.curr_func.base_name == conversion_method_name(expected_type) &&
        get_repr_type(ref c, val_res.type) == get_repr_type(ref c, expected_type)) {
        val_res.type = expected_type;
        return val_res;
    }
    if (callable_types_compatible(ref c, val_res.type, expected_type)) {
        val_res.type = expected_type;
        return val_res;
    }
    ...
    if (has_struct(variant_info) && expected_type == variant_info.type_id) {
        let boxed_type: Int = get_repr_type(ref c, val_res.type);
        ...
        let boxed_type_supported: Bool = val_res.type == TYPE_NULL ||
                                           ...
                                           is_ref_type(ref c, val_res.type) ||
                                           is_pointer_type(ref c, val_res.type) ||
                                           is_value_struct(ref c, val_res.type) ||
                                           boxed_enum;
        if (!boxed_type_supported) {
            throw_type_error(pos, "Type " + get_type_name(ref c, val_res.type) + " cannot be stored in Dict.");
            return CompileResult(reg="poison", type=TYPE_POISON);
        }

        let variant_llvm: String = variant_info.llvm_name;
        let box_ptr: String = emit_alloc_obj(ref c, "" + variant_payload_size(), "" + expected_type, variant_llvm + "*");
        ...
        let boxed_tag: UInt64 = type_fingerprint(ref c, val_res.type);
        ...
        if (val_res.type == TYPE_NULL || val_res.type == TYPE_NULLPTR) {
            ...
            let interface_ty: String = get_llvm_type_str(ref c, val_res.type);
            ...
            payload_low = next_reg(ref c);
            c.output_file.write(c.indent + payload_low + " = ptrtoint i8* " + object_ptr + " to i64\n");
            payload_high = next_reg(ref c);
            c.output_file.write(c.indent + payload_high + " = ptrtoint i8* " + table_ptr + " to i64\n");
            if (!val_res.owns_ref) {
                emit_retain(ref c, val_res.reg, val_res.type);
            }
        } else if (is_value_struct(ref c, val_res.type)) {
            let value_ty: String = get_llvm_type_str(ref c, val_res.type);
            ...
            let value_size: String = next_reg(ref c);
            c.output_file.write(c.indent + value_size + " = ptrtoint " + value_ty + "* " + size_ptr + " to " + size_ty + "\n");
            let value_ptr: String = emit_alloc_obj(ref c, value_size, "" + val_res.type, value_ty + "*");
            if (needs_drop(ref c, val_res.type) && !val_res.owns_ref) {
                emit_retain_value(ref c, val_res.reg, val_res.type);
            }
            c.output_file.write(c.indent + "store " + value_ty + " " + val_res.reg + ", " + value_ty + "* " + value_ptr + "\n");
            let erased: String = next_reg(ref c);
            ...
            let shifted_high: String = next_reg(ref c);
            ...
            let prim_ty: String = get_llvm_type_str(ref c, val_res.type);
            payload_low = next_reg(ref c);
            ...
            let fpext_reg: String = next_reg(ref c);
            ...
            if boxed_enum {
                ...
                let ptr_ty: String = get_llvm_type_str(ref c, val_res.type);
                c.output_file.write(c.indent + payload_low + " = ptrtoint " + ptr_ty + " " + val_res.reg + " to i64\n");
                if (is_ref_type(ref c, val_res.type) && !val_res.owns_ref) {
                    emit_retain(ref c, val_res.reg, val_res.type);
                }
            }
        }
    ...
    if (has_struct(variant_info_check) && val_res.type == variant_info_check.type_id) {
        let expected_repr: Int = get_repr_type(ref c, expected_type);
        ...
        let can_be_null: Bool = is_ref_type(ref c, expected_type) || is_pointer_type(ref c, expected_type);

        let is_null_ptr: String = next_reg(ref c);
        ...
        c.output_file.write("\n" + check_match_label + ":\n");
        let is_match: String = next_reg(ref c);
        ...
        if (expected_type == TYPE_GENERIC_STRUCT || expected_type == TYPE_GENERIC_CLASS) {
            ...
            while (candidate < c.type_counter) {
                ...
                if accepts {
                    let candidate_match: String = next_reg(ref c);
                    c.output_file.write(c.indent + candidate_match + " = icmp eq i64 " + type_id_reg + ", " + type_fingerprint(ref c, candidate) + "\n");
                    if (match_acc == "false") {
            ...
            c.output_file.write(c.indent + is_match + " = icmp eq i64 " + type_id_reg + ", " + type_fingerprint(ref c, expected_type) + "\n");
        }
        
        c.output_file.write(c.indent + "br i1 " + is_match + ", label %" + unbox_label + ", label %" + fail_label + "\n");

        c.output_file.write("\n" + fail_label + ":\n");
        emit_runtime_error(ref c, pos, "Dict value type mismatch or missing. Expected " + get_type_name(ref c, expected_type) + ".");

        c.output_file.write("\n" + unbox_label + ":\n");
        let payload_low_ptr: String = next_reg(ref c);
        c.output_file.write(c.indent + payload_low_ptr + " = getelementptr inbounds " + variant_llvm + ", " + variant_llvm + "* " + val_res.reg + ", i32 0, i32 1\n");
        let payload_low: String = next_reg(ref c);
        ...
        let payload_high: String = next_reg(ref c);
        ...
        if (expected_repr == TYPE_INT128 || expected_repr == TYPE_UINT128) {
            unboxed_reg = combine_i128_words(ref c, payload_low, payload_high);
        } else if (is_small_primitive_type(expected_repr)) {
            let prim_ty: String = get_llvm_type_str(ref c, expected_type);
            unboxed_reg = next_reg(ref c);
            c.output_file.write(c.indent + unboxed_reg + " = trunc i64 " + payload_low + " to " + prim_ty + "\n");
        } else if (expected_repr == TYPE_FLOAT32) {
            let cast_double: String = next_reg(ref c);
            ...
            if (has_struct(exp_s_info) && exp_s_info.is_interface) {
                ...
                let interface_ty: String = get_llvm_type_str(ref c, expected_type);
                let with_object: String = next_reg(ref c);
            ...
            } else if (is_value_struct(ref c, expected_type)) {
                let value_ty: String = get_llvm_type_str(ref c, expected_type);
                ...
                let ptr_ty: String = get_llvm_type_str(ref c, expected_type);
                unboxed_reg = next_reg(ref c);
        ...
        c.output_file.write("\n" + merge_label + ":\n");
        let final_val_reg: String = next_reg(ref c);
        let exp_ty_str: String = get_llvm_type_str(ref c, expected_type);
        
        if can_be_null {
            let zero_val: String = "0";
            if (expected_repr == TYPE_FLOAT) {
                zero_val = "0.0";
            } else if (is_nullable_reference_type(ref c, expected_type)) {
                zero_val = "null";
            }
        ...
        if (val_res.owns_ref) {
            if (is_ref_type(ref c, expected_type)) {
                emit_retain(ref c, final_val_reg, expected_type);
                result_owned = true;
            } else if (needs_drop(ref c, expected_type)) {
                emit_retain_value(ref c, final_val_reg, expected_type);
                result_owned = true;
            }
            emit_release_owned(ref c, val_res);
        }
    ...
    if (val_res.type == TYPE_NULLPTR) {
        if (is_pointer_type(ref c, expected_type)) {
    ...
    if (val_res.type == TYPE_NULL) {
        ...
        if (!is_nullable_reference_type(ref c, expected_type)) {
            throw_type_error(pos, "Type " + get_type_name(ref c, expected_type) + " cannot be null.");
            return CompileResult(reg="zeroinitializer", type=expected_type, origin_type=expected_type);
        }
        return CompileResult(reg="null", type=expected_type, origin_type=expected_type);
    }
    ...
    if (is_integer_type(expected_type) && is_integer_type(val_res.type)) {
        let widened_int: CompileResult = widen_int(ref c, val_res, expected_type);
        if (widened_int.type == expected_type) { return widened_int; }
    }
    if (expected_type == TYPE_CHAR && val_res.type == TYPE_BYTE) {
        let char_reg: String = next_reg(ref c);
    ...
    if (expected_type == TYPE_FLOAT && val_res.type == TYPE_INT) { return promote_to_float(ref c, val_res); }
    if (expected_type == TYPE_FLOAT && val_res.type == TYPE_LONG) { return promote_to_float(ref c, val_res); }
    if (expected_type == TYPE_FLOAT && val_res.type == TYPE_FLOAT32) { return promote_to_float(ref c, val_res); }
    ...
    if (has_struct(ex_info) && ex_info.is_interface) {
        if (has_struct(val_info) && val_info.is_class) {
            if (interface_uses_self(ref c, ex_info)) {
                throw_type_error(pos, "Interface '" + interface_diagnostic_name(ref c, expected_type) + "' uses Self and can only be used as a static constraint.");
                return CompileResult(reg="poison", type=TYPE_POISON);
            }
            if (!class_has_interface(ref c, val_info, ex_info)) {
            ...
            let obj_cast: String = next_reg(ref c);
            c.output_file.write(c.indent + obj_cast + " = bitcast " + val_info.llvm_name + "* " + val_res.reg + " to i8*\n");
            let s1: String = next_reg(ref c);
            let s2: String = next_reg(ref c);
    ...
    if (expected_type == TYPE_GENERIC_STRUCT || expected_type == TYPE_GENERIC_CLASS) {
        if (val_res.type >= 100) {
            if (expected_type == TYPE_GENERIC_STRUCT && is_value_struct(ref c, val_res.type)) {
                let value_ty: String = get_llvm_type_str(ref c, val_res.type);
                let size_ty: String = get_size_llvm_type();
                let size_ptr: String = next_reg(ref c);
                c.output_file.write(c.indent + size_ptr + " = getelementptr " + value_ty + ", " + value_ty + "* null, " + size_ty + " 1\n");
            ...
            let src_ty: String = get_llvm_type_str(ref c, val_res.type);
        ...
        if (has_struct(c.struct_id_map.lookup("" + expected_type)) || has_symbol(c.vector_base_map.lookup("" + expected_type))) {
            if (origin >= 100) {
                ...
                if (val_res.type == TYPE_GENERIC_CLASS) {
                    compatible = is_subclass(ref c, origin, expected_type);
                }
                if (!compatible) {
                    throw_type_error(pos, "Cannot restore " + get_type_name(ref c, val_res.type) + " as " + get_type_name(ref c, expected_type));
                    return CompileResult(reg="poison", type=TYPE_POISON);
                }
            } else {
                emit_erased_type_check(ref c, val_res.reg, expected_type, pos);
            }
            let cast_reg: String = next_reg(ref c);
            let dest_ty: String = get_llvm_type_str(ref c, expected_type);
            if (is_value_struct(ref c, expected_type)) {
                let value_ptr: String = next_reg(ref c);
                ...
                if (val_res.owns_ref && needs_drop(ref c, expected_type)) {
                    emit_retain_value(ref c, cast_reg, expected_type);
                    emit_release_owned(ref c, val_res);
                    owns_value = true;
                }
    ...
    if (is_pointer_type(ref c, expected_type) && is_pointer_type(ref c, val_res.type)) {
        if (is_void_ptr(ref c, expected_type) || is_void_ptr(ref c, val_res.type)) {
            let cast_reg: String = next_reg(ref c);
            let dest_ty: String = get_llvm_type_str(ref c, expected_type);
            let src_ty: String = get_llvm_type_str(ref c, val_res.type);
            if (dest_ty != src_ty) {
    ...
    if (is_void_ptr(ref c, expected_type)) {
    ...
    if (is_void_ptr(ref c, val_res.type) && expected_type != TYPE_NULL && expected_type != TYPE_NULLPTR && !is_void_ptr(ref c, expected_type)) {
    ...
    if (val_res.type == TYPE_GENERIC_FUNCTION && expected_type >= 100) {
        if (has_symbol(c.func_ret_map.lookup("" + expected_type))) {
            if (origin != 0 && !callable_types_compatible(ref c, origin, expected_type)) {
                throw_type_error(pos, "Cannot restore Function as " + get_type_name(ref c, expected_type));
                return CompileResult(reg="poison", type=TYPE_POISON);
            }
            emit_erased_type_check(ref c, val_res.reg, expected_type, pos);
    ...
    if (val_res.type == TYPE_GENERIC_METHOD && expected_type >= 100) {
        if (has_symbol(c.method_ret_map.lookup("" + expected_type))) {
            if (origin != 0 && !callable_types_compatible(ref c, origin, expected_type)) {
                throw_type_error(pos, "Cannot restore Method as " + get_type_name(ref c, expected_type));
                return CompileResult(reg="poison", type=TYPE_POISON);
            }
            emit_erased_type_check(ref c, val_res.reg, expected_type, pos);
            return CompileResult(reg=val_res.reg, type=expected_type, origin_type=origin, owns_ref=val_res.owns_ref);
    ...
    if (val_res.type >= 100 && expected_type >= 100) {
        if (is_subclass(ref c, val_res.type, expected_type)) {
            let cast_reg: String = next_reg(ref c);
            let dest_ty: String = get_llvm_type_str(ref c, expected_type);
            let src_ty: String = get_llvm_type_str(ref c, val_res.type);
            c.output_file.write(c.indent + cast_reg + " = bitcast " + src_ty + " " + val_res.reg + " to " + dest_ty + "\n");
    ...
    if (has_array_info(expected_arr) && expected_arr.size == -1) {
        ...
        if (has_array_info(val_arr) && val_arr.size > 0 && val_arr.base_type == elem_type) {
            ...
            return emit_slice_copy(ref c, elem_type, data_ptr, "0", "" + val_arr.size, pos);
        }

        let val_vec: SymbolInfo = c.vector_base_map.lookup("" + val_res.type);
        if (has_symbol(val_vec) && val_vec.type == elem_type) {
            let vec_struct_ty: String = get_vector_llvm_type(ref c, elem_type);
            let size_ty: String = get_size_llvm_type();
            let size_ptr: String = next_reg(ref c);
            ...
            let data_ptr_ptr: String = next_reg(ref c);
            ...
            if (size_ty != "i32") {
                size_i32 = next_reg(ref c);
                c.output_file.write(c.indent + size_i32 + " = trunc " + size_ty + " " + size_val + " to i32\n");
            }
            let copied: CompileResult = emit_slice_copy(ref c, elem_type, data_ptr, "0", size_i32, pos);
            emit_release_owned(ref c, val_res);
            return copied;
        }
    }

    throw_type_error(pos, "Type mismatch. Expected " + get_type_name(ref c, expected_type) + ", got " + get_type_name(ref c, val_res.type));
    return CompileResult(reg="0", type=expected_type, origin_type=expected_type);
}

func convert_to_string(ref c: Compiler, res: CompileResult) -> CompileResult {
    if (res.type == TYPE_STRING) { return res; }

    if (res.type == TYPE_INT8 || res.type == TYPE_INT16 || res.type == TYPE_UINT16) {
        res = promote_to_int(ref c, res);
    }
    if (res.type == TYPE_UINT32) {
        res = promote_to_long(ref c, res);
    }
    if (res.type == TYPE_FLOAT32) {
        res = promote_to_float(ref c, res);
    }
    if (res.type == TYPE_INTSIZE && get_target_pointer_bits() < 64) {
        let widened: String = next_reg(ref c);
    ...
    if (res.type == TYPE_INT128 || res.type == TYPE_UINT128) {
        ...
        let format_hook: String = get_mangled_symbol(ref c, hook_name, no_position());
    ...
    if (res.type == TYPE_UINT64 || res.type == TYPE_UINTSIZE) {
        let format_hook: String = get_mangled_symbol(ref c, "format_uint64", no_position());
        let result: String = next_reg(ref c);
    ...
    if (res.type == TYPE_INT || res.type == TYPE_BYTE) {
        ...
        if (res.type == TYPE_BYTE) {
            val_reg = next_reg(ref c);
            c.output_file.write(c.indent + val_reg + " = zext i8 " + res.reg + " to i32\n");
        }

        let format_hook: String = get_mangled_symbol(ref c, "format_int", no_position());
        let result: String = next_reg(ref c);
    ...
    if (res.type == TYPE_LONG) {
        let format_hook: String = get_mangled_symbol(ref c, "format_long", no_position());
        let result: String = next_reg(ref c);
        c.output_file.write(c.indent + result + " = call %struct.$String* @" + format_hook + "(i64 " + res.reg + ")\n");
        return CompileResult(reg=result, type=TYPE_STRING, owns_ref=true);
    }
    ...
    if (res.type == TYPE_FLOAT) {
        let format_hook: String = get_mangled_symbol(ref c, "format_float", no_position());
        let result: String = next_reg(ref c);
    ...
    if (res.type == TYPE_BOOL) {
        let true_id: Int = register_string_constant(ref c, "true");
        let false_id: Int = register_string_constant(ref c, "false");
        let result: String = next_reg(ref c);
    ...
    if (res.type == TYPE_CHAR) {
        let format_hook: String = get_mangled_symbol(ref c, "utf8_encode_char", no_position());
        let result: String = next_reg(ref c);
    ...
    let null_id: Int = register_string_constant(ref c, "null");
...
func is_numeric_literal_expression(ref c: Compiler, node: NodeID) -> Bool {
...
func validate_explicit_literal_cast(ref c: Compiler, node: NodeID, target_type: Int, pos: Position) -> Bool {
    ...
    let target_name: String = get_type_name(ref c, target_type);
    if is_float_literal {
...
func append_cast_condition(ref c: Compiler, current: String, next: String) -> String {
...
func emit_integer_cast_check(ref c: Compiler, value: String, source_type: Int, target_type: Int) -> String {
    let llvm_type: String = get_llvm_type_str(ref c, source_type);
    ...
    if (target_type == TYPE_BOOL) {
        let is_zero: String = next_reg(ref c);
        let is_one: String = next_reg(ref c);
        let result: String = next_reg(ref c);
    ...
    if (target_type == TYPE_CHAR) {
        if signed_source {
            let non_negative: String = next_reg(ref c);
            c.output_file.write(c.indent + non_negative + " = icmp sge " + llvm_type + " " + value + ", 0\n");
            valid = append_cast_condition(ref c, valid, non_negative);
        }
        if (source_bits >= 32) {
            let below_limit: String = next_reg(ref c);
            ...
            valid = append_cast_condition(ref c, valid, below_limit);
        }
        let can_reach_surrogates: Bool = source_bits >= 32 ||
            (!signed_source && source_bits >= 16);
        if can_reach_surrogates {
            let below_surrogates: String = next_reg(ref c);
            let above_surrogates: String = next_reg(ref c);
            let outside_surrogates: String = next_reg(ref c);
            ...
            valid = append_cast_condition(ref c, valid, outside_surrogates);
        }
...
func emit_float_cast_check(ref c: Compiler, value: String, source_type: Int, target_type: Int) -> String {
    let llvm_type: String = get_llvm_type_str(ref c, source_type);
    if (target_type == TYPE_BOOL) {
        let is_zero: String = next_reg(ref c);
        let is_one: String = next_reg(ref c);
        let valid: String = next_reg(ref c);
    ...
    let valid: String = "";
    let lower: String = next_reg(ref c);
    let upper: String = next_reg(ref c);
    ...
    valid = append_cast_condition(ref c, valid, lower);
    valid = append_cast_condition(ref c, valid, upper);

    if (target_type == TYPE_CHAR) {
        let before_surrogates: String = next_reg(ref c);
        let after_surrogates: String = next_reg(ref c);
        let outside_surrogates: String = next_reg(ref c);
...
func standard_overflow_error(ref c: Compiler, pos: Position) -> CompileResult {
    let i: Int = 0;
    while (i < c.error_types.length()) {
        ...
        if (info.compiler_link_name == "Error") {
            ...
            if (has_field(field)) {
                return emit_error_value(ref c, CompileResult(reg="" + field.offset, type=info.type_id, origin_type=info.type_id), pos);
            }
        }
        i += 1;
    }
...
func emit_fallible_cast(ref c: Compiler, value: CompileResult, valid: String, target_type: Int, pos: Position) -> CompileResult {
    let fail_label: String = next_label(ref c);
    let success_label: String = next_label(ref c);
    let end_label: String = next_label(ref c);
    let fallible_type: Int = get_fallible_type_id(ref c, target_type);
    let fallible_llvm: String = get_llvm_type_str(ref c, fallible_type);
    let target_llvm: String = get_llvm_type_str(ref c, target_type);

    c.output_file.write(c.indent + "br i1 " + valid + ", label %" + success_label + ", label %" + fail_label + "\n");

    c.output_file.write("\n" + fail_label + ":\n");
    let error_value: CompileResult = standard_overflow_error(ref c, pos);
    let fail_1: String = next_reg(ref c);
    let fail_2: String = next_reg(ref c);
    ...
    c.output_file.write("\n" + success_label + ":\n");
    let converted: CompileResult = compile_type_cast(ref c, value, target_type, pos);
    let success_1: String = next_reg(ref c);
    let success_2: String = next_reg(ref c);
    let success_3: String = next_reg(ref c);
...
func unwrap_conversion_or_panic(ref c: Compiler, value: CompileResult, source_type: Int, target_type: Int, pos: Position) -> CompileResult {
    let fallible_llvm: String = get_llvm_type_str(ref c, value.type);
    ...
    c.output_file.write("\n" + fail_label + ":\n");
    emit_runtime_error(ref c, pos, "conversion from " + get_type_name(ref c, source_type) + " to " + get_type_name(ref c, target_type) + " failed");

    c.output_file.write("\n" + success_label + ":\n");
    let result: String = next_reg(ref c);
    c.output_file.write(c.indent + result + " = extractvalue " + fallible_llvm + " " + value.reg + ", 2\n");
    return CompileResult(reg=result, type=target_type, origin_type=0, owns_ref=value.owns_ref && needs_drop(ref c, target_type));
}

func compile_explicit_type_cast(ref c: Compiler, value: CompileResult, target_type: Int, pos: Position, preserve_failure: Bool) -> CompileResult {
    if (value.type == TYPE_POISON) { return value; }

    if (value.type == TYPE_ANY_ERROR && is_integer_type(target_type)) {
        let code: String = next_reg(ref c);
        c.output_file.write(c.indent + code + " = extractvalue { i64, i32 } " + value.reg + ", 1\n");
        return compile_explicit_type_cast(ref c, CompileResult(reg=code, type=TYPE_INT), target_type, pos, preserve_failure);
    }

    if (!needs_explicit_cast(ref c, value.type, target_type)) {
        return compile_type_cast(ref c, value, target_type, pos);
    }
    ...
    if (is_integer_type(check_type)) {
        valid = emit_integer_cast_check(ref c, value.reg, check_type, target_type);
    } else if (check_type == TYPE_FLOAT || check_type == TYPE_FLOAT32) {
        valid = emit_float_cast_check(ref c, value.reg, check_type, target_type);
    } else {
        return compile_type_cast(ref c, value, target_type, pos);
    }

    if preserve_failure {
        return emit_fallible_cast(ref c, value, valid, target_type, pos);
    }
    ...
    c.output_file.write(c.indent + "br i1 " + valid + ", label %" + success_label + ", label %" + fail_label + "\n");
    c.output_file.write("\n" + fail_label + ":\n");
    emit_runtime_error(ref c, pos, "conversion from " + get_type_name(ref c, value.type) + " to " + get_type_name(ref c, target_type) + " is out of range");
    c.output_file.write("\n" + success_label + ":\n");
    return compile_type_cast(ref c, value, target_type, pos);
}

func compile_type_cast(ref c: Compiler, val_res: CompileResult, target_type: Int, pos: Position) -> CompileResult {
    if (val_res.type == TYPE_POISON) { return val_res; }
    if (val_res.type == target_type) { return val_res; }

    let source_repr: Int = get_repr_type(ref c, val_res.type);
    let target_repr: Int = get_repr_type(ref c, target_type);
    if (source_repr != val_res.type || target_repr != target_type) {
        ...
        let converted: CompileResult = compile_type_cast(ref c, raw, target_repr, pos);
        converted.type = target_type;
        return converted;
    }

    let src_ty_str: String = get_llvm_type_str(ref c, val_res.type);
    let dst_ty_str: String = get_llvm_type_str(ref c, target_type);
    let res_reg: String = next_reg(ref c);

    if (target_type == TYPE_STRING) {
        return convert_to_string(ref c, val_res);
    }
    ...
    if (val_res.type == TYPE_ANY_ERROR && is_integer_type(target_type)) {
        ...
        return compile_type_cast(ref c, CompileResult(reg=code, type=TYPE_INT), target_type, pos);
    }
    ...
    let src_is_ptr: Bool = is_pointer_type(ref c, val_res.type) || val_res.type == TYPE_STRING ||
                           val_res.type == TYPE_ANYPTR || val_res.type == TYPE_NULLPTR || src_is_erased_struct;
    let dst_is_ptr: Bool = is_pointer_type(ref c, target_type) || target_type == TYPE_STRING || target_type == TYPE_ANYPTR;
