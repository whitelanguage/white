...
func is_printable_type(ref c: Compiler, type_id: Int) -> Bool {
    type_id = get_repr_type(ref c, type_id);
    if (type_id == TYPE_POISON || is_fallible_type(ref c, type_id)) { return false; }
    if (type_id == TYPE_STRING || type_id == TYPE_CHAR || type_id == TYPE_ANY_ERROR ||
        ...
        is_pointer_type(ref c, type_id) || type_id == TYPE_GENERIC_STRUCT || type_id == TYPE_GENERIC_CLASS) {
        return true;
    }
...
func class_has_named_interface(ref c: Compiler, info: StructInfo, name: String) -> Bool {
...
func interface_has_name(ref c: Compiler, info: StructInfo, name: String) -> Bool {
...
func compile_interface_display(ref c: Compiler, reg: String, info: StructInfo, pos: Position) -> Bool {
    if (!interface_has_name(ref c, info, "formatting.Display")) { return false; }
    ...
    compile_print(ref c, "null", TYPE_NULL, pos, TYPE_NULL);
    ...
    c.output_file.write(c.indent + method_ptr + " = bitcast i8* " + method_raw + " to " + interface_method_sig(ref c, info, method_node) + "\n");

    let text: String = next_reg(ref c);
    c.output_file.write(c.indent + text + " = call %struct.$String* " + method_ptr + "(i8* " + object + ")\n");

    compile_print(ref c, text, TYPE_STRING, pos, TYPE_STRING);
    emit_release_owned(ref c, CompileResult(reg=text, type=TYPE_STRING, owns_ref=true));
...
func compile_display(ref c: Compiler, reg: String, info: StructInfo, pos: Position) -> Bool {
    if (!info.is_class || !class_has_named_interface(ref c, info, "formatting.Display")) { return false; }
    ...
    queue_generic_class_method(ref c, info, method_info.base_name);
    ...
    let signature: String = get_func_sig_str(ref c, method_info);
...
func compile_print(ref c: Compiler, reg: String, type_id: Int, pos: Position, origin_id: Int) -> Void {
    type_id = get_repr_type(ref c, type_id);
    ...
    if (type_id == TYPE_INT128 || type_id == TYPE_UINT128) {
        let formatted: CompileResult = convert_to_string(ref c, CompileResult(reg=reg, type=type_id, origin_type=origin_id));
        compile_print(ref c, formatted.reg, TYPE_STRING, pos, TYPE_STRING);
        emit_release_owned(ref c, formatted);
        return;
    }

    if (type_id == TYPE_STRING) {
        let hook_raw_str: String = get_mangled_symbol(ref c, "print_bytes", pos);
        let label_null: String = next_label(ref c);
        let label_value: String = next_label(ref c);
        ...
        c.output_file.write("\n" + label_value + ":\n");
        let struct_reg: String = next_reg(ref c);
        let ptr_reg: String = next_reg(ref c);
        let len_field: String = next_reg(ref c);
        let len_reg: String = next_reg(ref c);
    ...
    if (type_id == TYPE_CHAR) {
        let hook_char: String = get_mangled_symbol(ref c, "print_char", pos);
        c.output_file.write(c.indent + "call void @" + hook_char + "(i32 " + reg + ")\n");
        return;
    }

    if (type_id == TYPE_ANY_ERROR) {
        compile_print_error_internal(ref c, reg, pos);
        return;
    }
    ...
    if (is_primitive_type(type_id)) {
        if (type_id == TYPE_INT || type_id == TYPE_INT8 || type_id == TYPE_INT16 || type_id == TYPE_UINT16) {
            ...
            if (type_id != TYPE_INT) { temp_res = promote_to_int(ref c, temp_res); }
            let hook_int: String = get_mangled_symbol(ref c, "print_int", pos);
        ...
        if (type_id == TYPE_LONG || type_id == TYPE_UINT32 || type_id == TYPE_INTSIZE) {
            let temp_res: CompileResult = CompileResult(reg=reg, type=type_id, origin_type=origin_id);
            if (type_id != TYPE_LONG) { temp_res = promote_to_long(ref c, temp_res); }
            let hook_long: String = get_mangled_symbol(ref c, "print_long", pos);
            c.output_file.write(c.indent + "call void @" + hook_long + "(i64 " + temp_res.reg + ")\n");
            return;
        }
        if (type_id == TYPE_FLOAT || type_id == TYPE_FLOAT32) {
            let temp_res: CompileResult = CompileResult(reg=reg, type=type_id, origin_type=origin_id);
            if (type_id != TYPE_FLOAT) { temp_res = promote_to_float(ref c, temp_res); }
            let hook_float: String = get_mangled_symbol(ref c, "print_float", pos);
            c.output_file.write(c.indent + "call void @" + hook_float + "(double " + temp_res.reg + ")\n");
            return;
        }
        if (type_id == TYPE_BOOL) {
            let hook_bool: String = get_mangled_symbol(ref c, "print_bool", pos);
    ...
    if (type_id == TYPE_GENERIC_STRUCT || type_id == TYPE_GENERIC_CLASS) {
        if (origin_id >= 100) {
            ...
            if (has_struct(s_info_real)) {
                ...
                if (compile_display(ref c, cast_reg, s_info_real, pos)) { return; }
                compile_print_struct_internal(ref c, cast_reg, s_info_real, pos);
                return;
            }
        }
        let ptr_i64: String = next_reg(ref c);
        let hook_long: String = get_mangled_symbol(ref c, "print_long", pos);
    ...
    if (type_id >= 100) {
        ...
        if (has_struct(s_info)) {
            if (compile_interface_display(ref c, reg, s_info, pos)) { return; }
            if (compile_display(ref c, reg, s_info, pos)) { return; }
            if (s_info.name == "$Variant") {
                compile_print_variant_internal(ref c, reg, s_info, pos);
                return;
            }
            if (s_info.is_enum) {
                compile_print_enum_internal(ref c, reg, s_info, pos);
                return;
            }
            compile_print_struct_internal(ref c, reg, s_info, pos);
            return;
        }
        
        let v_info: SymbolInfo = c.vector_base_map.lookup("" + type_id);
        if (has_symbol(v_info)) {
            compile_print_vector_internal(ref c, reg, v_info, pos);
            return;
        }

        let arr_info: ArrayInfo = c.array_info_map.lookup("" + type_id);
        if (has_array_info(arr_info)) {
            compile_print_array_internal(ref c, reg, type_id, arr_info, pos);
            return;
        }
    }

    throw_type_error(pos, "Type " + get_type_name(ref c, type_id) + " cannot be printed.");
}

func compile_print_error_internal(ref c: Compiler, error_reg: String, pos: Position) -> Void {
    let domain: String = next_reg(ref c);
    ...
    while (i < c.error_types.length()) {
        ...
        let match_label: String = next_label(ref c);
        let next_type_label: String = next_label(ref c);
        let matches: String = next_reg(ref c);
        c.output_file.write(c.indent + matches + " = icmp eq i64 " + domain + ", " + type_fingerprint(ref c, info.type_id) + "\n");
        c.output_file.write(c.indent + "br i1 " + matches + ", label %" + match_label + ", label %" + next_type_label + "\n");

        c.output_file.write("\n" + match_label + ":\n");
        compile_print_enum_internal(ref c, code, info, pos);
        c.output_file.write(c.indent + "br label %" + end_label + "\n");
    ...
    let prefix_id: Int = register_string_constant(ref c, prefix);
...
func compile_print_enum_internal(ref c: Compiler, enum_reg: String, s_info: StructInfo, pos: Position) -> Void {
    let hook_raw_str: String = get_mangled_symbol(ref c, "print_bytes", pos);
    let default_label: String = next_label(ref c);
    let end_label: String = next_label(ref c);
    ...
    while (i < len) {
        ...
        let lbl: String = next_label(ref c);
        labels.append(lbl);
    ...
    while (i < len) {
        ...
        let id: Int = register_string_constant(ref c, name_str);
    ...
    let unk_id: Int = register_string_constant(ref c, unk_str);
...
func compile_print_struct_internal(ref c: Compiler, obj_reg: String, s_info: StructInfo, pos: Position) -> Void {
    let hook_raw_str: String = get_mangled_symbol(ref c, "print_bytes", pos);
    let object_ptr: String = obj_reg;
    if (!s_info.is_class) {
        object_ptr = next_reg(ref c);
    ...
    let header_id: Int = register_string_constant(ref c, header);
    ...
    while (f_idx < f_len) {
        ...
        let fn_id: Int = register_string_constant(ref c, f_name_eq);
        ...
        let f_val_reg: String = next_reg(ref c);
        c.output_file.write(c.indent + f_val_reg + " = load " + f_curr.llvm_type + ", " + f_curr.llvm_type + "* " + f_ptr + "\n");
        compile_print(ref c, f_val_reg, f_curr.type, pos, f_curr.type); 
...
func compile_print_vector_internal(ref c: Compiler, vec_reg: String, v_info: SymbolInfo, pos: Position) -> Void {
    let hook_raw_str: String = get_mangled_symbol(ref c, "print_bytes", pos);
    ...
    let label_sep: String = next_label(ref c);
    let label_end: String = next_label(ref c);

    let idx_ptr: String = next_reg(ref c);
    ...
    let val: String = next_reg(ref c);
    c.output_file.write(c.indent + val + " = load " + elem_ty_str + ", " + elem_ty_str + "* " + slot + "\n");
    
    compile_print(ref c, val, elem_type, pos, elem_type);

    let next_idx: String = next_reg(ref c);
    ...
    let is_not_last: String = next_reg(ref c);
...
func compile_print_array_internal(ref c: Compiler, arr_reg: String, type_id: Int, arr_info: ArrayInfo, pos: Position) -> Void {
    let hook_raw_str: String = get_mangled_symbol(ref c, "print_bytes", pos);
    ...
    if (arr_info.size == -1) {
        slice_parts = emit_slice_parts(ref c, arr_reg, type_id, pos);
        let slice_length: String = emit_size_to_int(ref c, slice_parts.length);
    ...
    compile_print(ref c, val_reg, elem_type, pos, elem_type);
...
func compile_print_variant_internal(ref c: Compiler, variant_reg: String, v_info: StructInfo, pos: Position) -> Void {
    let hook_raw_str: String = get_mangled_symbol(ref c, "print_bytes", pos);
    ...
    c.output_file.write("    i64 " + type_fingerprint(ref c, TYPE_INT) + ", label %" + label_int + "\n");
    c.output_file.write("    i64 " + type_fingerprint(ref c, TYPE_LONG) + ", label %" + label_long + "\n");
    c.output_file.write("    i64 " + type_fingerprint(ref c, TYPE_FLOAT) + ", label %" + label_float + "\n");
    c.output_file.write("    i64 " + type_fingerprint(ref c, TYPE_BOOL) + ", label %" + label_bool + "\n");
    c.output_file.write("    i64 " + type_fingerprint(ref c, TYPE_STRING) + ", label %" + label_string + "\n");
    c.output_file.write("    i64 " + type_fingerprint(ref c, TYPE_CHAR) + ", label %" + label_char + "\n");
    c.output_file.write("    i64 " + type_fingerprint(ref c, TYPE_INT128) + ", label %" + label_int128 + "\n");
    c.output_file.write("    i64 " + type_fingerprint(ref c, TYPE_UINT128) + ", label %" + label_uint128 + "\n");
    c.output_file.write("  ]\n");
    ...
    c.output_file.write("\n" + label_int + ":\n");
    let unboxed_int: String = next_reg(ref c);
    c.output_file.write(c.indent + unboxed_int + " = trunc i64 " + payload_low + " to i32\n");
    compile_print(ref c, unboxed_int, TYPE_INT, pos, 0);
    c.output_file.write(c.indent + "br label %" + label_end + "\n");

    c.output_file.write("\n" + label_long + ":\n");
    compile_print(ref c, payload_low, TYPE_LONG, pos, 0);
    c.output_file.write(c.indent + "br label %" + label_end + "\n");

    c.output_file.write("\n" + label_float + ":\n");
    let unboxed_float: String = next_reg(ref c);
    c.output_file.write(c.indent + unboxed_float + " = bitcast i64 " + payload_low + " to double\n");
    compile_print(ref c, unboxed_float, TYPE_FLOAT, pos, 0);
    c.output_file.write(c.indent + "br label %" + label_end + "\n");
    
    c.output_file.write("\n" + label_bool + ":\n");
    let unboxed_bool: String = next_reg(ref c);
    c.output_file.write(c.indent + unboxed_bool + " = trunc i64 " + payload_low + " to i1\n");
    compile_print(ref c, unboxed_bool, TYPE_BOOL, pos, 0);
    c.output_file.write(c.indent + "br label %" + label_end + "\n");

    c.output_file.write("\n" + label_string + ":\n");
    let unboxed_str: String = next_reg(ref c);
    c.output_file.write(c.indent + unboxed_str + " = inttoptr i64 " + payload_low + " to %struct.$String*\n");
    compile_print(ref c, unboxed_str, TYPE_STRING, pos, 0);
    c.output_file.write(c.indent + "br label %" + label_end + "\n");

    c.output_file.write("\n" + label_char + ":\n");
    let unboxed_char: String = next_reg(ref c);
    c.output_file.write(c.indent + unboxed_char + " = trunc i64 " + payload_low + " to i32\n");
    compile_print(ref c, unboxed_char, TYPE_CHAR, pos, 0);
    c.output_file.write(c.indent + "br label %" + label_end + "\n");

    c.output_file.write("\n" + label_int128 + ":\n");
    let unboxed_int128: String = combine_i128_words(ref c, payload_low, payload_high);
    compile_print(ref c, unboxed_int128, TYPE_INT128, pos, 0);
    c.output_file.write(c.indent + "br label %" + label_end + "\n");

    c.output_file.write("\n" + label_uint128 + ":\n");
    let unboxed_uint128: String = combine_i128_words(ref c, payload_low, payload_high);
    compile_print(ref c, unboxed_uint128, TYPE_UINT128, pos, 0);
    c.output_file.write(c.indent + "br label %" + label_end + "\n");
